...
use BC\WP\Contracts\Foundation\Application;
use BC\WP\Contracts\View\Factory;
use BC\WP\Filesystem\Filesystem;
use BC\WP\Support\Str;
use BC\WP\View\AnonymousComponent;
use BC\WP\View\DynamicComponent;
use BC\WP\View\ViewFinderInterface;
...
class ComponentTagCompiler
{
     ...
     *
     * @var \BC\WP\View\Compilers\BladeCompiler
     */
    protected $blade;
     ...
     * @param  \BC\WP\View\Compilers\BladeCompiler|null  $blade
     * @return void
     */
    ...
    protected function componentString(string $component, array $attributes)
    {
        ...
        if (! class_exists($class)) {
            $view = Str::startsWith($component, 'mail::')
                ? "\$__env->getContainer()->make(BC\WP\\View\\Factory::class)->make('{$component}')"
                : "'$class'";
...
<?php if (isset($attributes) && $attributes instanceof BC\WP\View\ComponentAttributeBag && $constructor = (new ReflectionClass('.$class.'::class))->getConstructor()): ?>
...
<?php $component->withAttributes(['.$this->attributesToString($attributes->all(), $escapeAttributes = $class !== DynamicComponent::class).']); ?>';
     ...
     *
     * @param  \BC\WP\Contracts\View\Factory  $viewFactory
     * @param  string  $component
     * @return string|null
     */
        ...
        return preg_replace_callback(
            '/@(class)(\( ( (?>[^()]+) | (?2) )* \))/x', function ($match) {
                if ($match[1] === 'class') {
                    ...
                    return ":class=\"\BC\WP\Support\Arr::toCssClasses{$match[2]}\"";
                }
        ...
        return preg_replace_callback(
            '/@(style)(\( ( (?>[^()]+) | (?2) )* \))/x', function ($match) {
                if ($match[1] === 'style') {
                    ...
                    return ":style=\"\BC\WP\Support\Arr::toCssStyles{$match[2]}\"";
                }
        ...
        return collect($attributes)
                ->map(function (string $value, string $attribute) use ($escapeBound) {
                    return $escapeBound && isset($this->boundAttributes[$attribute]) && $value !== 'true' && ! is_numeric($value)
                                ? "'{$attribute}' => \BC\WP\View\Compilers\BladeCompiler::sanitizeComponentAttribute({$value})"
                                : "'{$attribute}' => {$value}";
                })
                ->implode(',');
    }
